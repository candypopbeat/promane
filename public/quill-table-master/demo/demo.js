...
var TableCell = function (_ContainBlot) {
    ...
    (0, _createClass3.default)(TableCell, [{
        ...
        value: function formats() {
            // We don't inherit from FormatBlot
            return (0, _defineProperty3.default)({}, this.statics.blotName, this.domNode.getAttribute('table_id') + '|' + this.domNode.getAttribute('row_id') + '|' + this.domNode.getAttribute('cell_id') + '|' + this.domNode.getAttribute('width'));
        }
    }, {
        key: 'optimize',
        value: function optimize(context) {
...
var TableModule = function TableModule(quill, options) {
    ...
    clipboard.addMatcher('TD', function (node, delta) {
        return delta.compose(new _quillDelta2.default().retain(delta.length(), {
            td: node.getAttribute('table_id') + '|' + node.getAttribute('row_id') + '|' + node.getAttribute('cell_id') + '|' + node.getAttribute('width')
        }));
    });
};
